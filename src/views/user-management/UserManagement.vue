...
const paginated = computed(() =>
  filtered.value.slice((currentPage.value - 1) * itemsPerPage.value, currentPage.value * itemsPerPage.value)
);
...
const openEditRole = (user: ManagedUser) => {
  roleForm.value = { _id: user._id, name: user.name, email: user.email, role: user.role };
};

const onAdd = async () => {
  isSubmitting.value = true;
  try { await handleAddUser(createForm.value); resetCreateForm(); }
  finally { isSubmitting.value = false; }
};

const onEditRole = async () => {
  isEditSubmitting.value = true;
  try { await handleEditUserRole(roleForm.value); resetRoleForm(); }
...
<template>
  <DashboardLayout>
    <Breadcrumb
      ...
      :items="[{ name: 'Dashboard', href: '/dashboard' }, { name: 'Manajemen User' }]"
    />
    ...
    <div class="card">
      ...
      <div class="card-body">
        <div class="row mb-3">
          ...
          <div class="col-md-2">
            <button
              ...
              @click="searchQuery = ''; selectedRole = ''; currentPage = 1"
            >
        ...
        <div class="d-flex justify-content-end mb-3">
          <ModalButton variant="info" font="medium" size="lg" dataBsTarget="add-user-management">
            <i class="ti ti-plus me-1"></i>Tambah User
          </ModalButton>
        </div>

        <UserCreateModal
          modalId="add-user-management"
            ...
            <tbody>
              ...
              >
                ...
                <td>
                  <span
                    class="badge"
                    :class="user.role === 'super_admin' ? 'bg-danger' : user.role === 'member' ? 'bg-primary' : 'bg-secondary'"
                  >
                    {{ user.role }}
                  </span>
                </td>
                ...
                <td>
                  <div class="d-flex justify-content-center gap-1">
                    <ModalButton
                      ...
                      font="medium"
                      size="sm"
                      dataBsTarget="edit-user-management"
