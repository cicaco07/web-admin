...
const props = defineProps<{ targetType: PatchTargetType }>();
...
const hasMasterTargets = computed(() => ['HERO', 'ITEM'].includes(props.targetType));
const targetOptions = computed(() => {
  if (props.targetType === 'HERO') return heroesResult.value?.heroes?.items || [];
  if (props.targetType === 'ITEM') return itemsResult.value?.items || [];
  return [];
});
...
const labels: Record<PatchTargetType, string> = {
  HERO: 'Hero', ITEM: 'Item', BATTLEFIELD: 'Battlefield', SYSTEM: 'System', GAME_MODE: 'Game Mode',
};
const targetLabel = computed(() => labels[props.targetType]);
const pageTitle = computed(() => `${targetLabel.value} Patch History`);
    ...
    <div class="card">
      <div class="border-bottom title-part-padding patch-card-header">
        <div>
          ...
          <small class="text-muted">Lihat riwayat perubahan {{ targetLabel.toLowerCase() }} dari waktu ke waktu.</small>
        </div>
      </div>
      <div class="card-body">
        <div class="row g-2 mb-3">
          <div v-if="hasMasterTargets" class="col-md-8">
            <select v-model="selectedTarget" class="form-select">
              <option value="">Semua {{ targetLabel }}</option>
              <option v-for="option in targetOptions" :key="option._id" :value="option._id">{{ option.name }}</option>
            </select>
          </div>
          <div :class="hasMasterTargets ? 'col-md-4' : 'col-md-12'" class="d-flex align-items-center">
        ...
        <div v-if="loading" class="text-center py-4">Memuat riwayat...</div>
        <div v-else class="timeline-list">
          <div v-for="change in filteredChanges" :key="change._id" class="border rounded p-3 mb-3 patch-history-card">
            ...
            <p class="mb-2 lh-base">{{ change.description }}</p>
            <div v-if="change.details?.length" class="table-responsive">
              <table class="table table-sm table-bordered mb-0">
                <thead class="table-light"><tr><th>Field</th><th>Sebelum</th><th>Sesudah</th><th>Data Mentah</th></tr></thead>
          ...
          <div v-if="filteredChanges.length === 0" class="patch-state"><i class="ti ti-history-off"></i>Belum ada riwayat untuk filter ini.</div>
        </div>
      </div>
    </div>
  </DashboardLayout>
