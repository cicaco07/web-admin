...
<template>
  <DashboardLayout>
    ...
    <div class="card"><div class="border-bottom title-part-padding"><h4 class="card-title mb-0">Patch Note Hero</h4></div><div class="card-body">
      ...
      <div class="d-flex justify-content-end mb-3"><ModalButton variant="info" font="medium" size="lg" dataBsTarget="add-hero-patch-note"><i class="ti ti-plus me-1"></i>Tambah Hero Patch</ModalButton></div>
      ...
      <div class="table-responsive"><table class="table table-bordered table-hover"><thead class="table-light"><tr class="text-center"><th>No</th><th>Patch</th><th>Judul</th><th>Jumlah Hero</th><th>Deskripsi</th><th>Aksi</th></tr></thead><tbody>
        <tr v-if="loading"><td colspan="6" class="text-center py-4">Memuat data...</td></tr>
        <tr v-for="(row, index) in paginated" :key="row._id" class="text-center align-middle" v-show="!loading"><td>{{ (currentPage - 1) * itemsPerPage + index + 1 }}</td><td>{{ row.patch.name }}</td><td>{{ row.title }}</td><td>{{ row.hero_changes?.length || 0 }}</td><td class="text-start">{{ row.description }}</td><td><div class="d-flex justify-content-center gap-1"><ModalButton variant="warning" font="medium" size="sm" dataBsTarget="edit-hero-patch-note" @click="openEdit(row)"><i class="ti ti-edit"></i></ModalButton><Button variant="danger" font="medium" size="sm" type="button" @click="handleDeleteHeroPatchNote(row._id)"><i class="ti ti-trash"></i></Button></div></td></tr>
